...
interface FranchiseOrderSummaryCardsProps {
...
export function FranchiseOrderSummaryCards({ orders, totalCount, isAdmin = false }: FranchiseOrderSummaryCardsProps) {
  ...
  return (
    <div className={`grid grid-cols-2 ${isAdmin ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-3 md:gap-4 overflow-hidden`}>
      {cards.map((card) => (
